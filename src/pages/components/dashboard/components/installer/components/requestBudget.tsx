...
export default function RequestQuoteModal({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFiles((prev) => [...prev, ...newFiles.slice(0, totalAllowed)]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append("protection", protections.join(", "));
    ...
    files
      .slice(0, 2)
      .forEach((f, i) => formData.append(`photo${i + 1}`, f.file));

    try {
      console.groupCollapsed("FormData -> POST /api/v1/budget/");
      ...
      toast.success(
        "Orçamento enviado com sucesso! Você pode visualiza-lo em Meus orçamentos"
      );
      setFiles([]);
      onClose();
    } catch (err) {
          ...
          <form id="quote-form" className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                name="station_count"
                type="number"
                placeholder="Qtd. estações"
                className=" p-2 rounded border border-zinc-600 bg-zinc-800 text-white w-40"
                min={1}
                required
              />
            </div>
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="name" className="text-sm font-medium">
                Distância ate o quadro (m)
              </label>
              <input
                name="distance"
                type="number"
                placeholder="Distância até o quadro (m)"
                className="p-2 rounded border border-zinc-600 bg-zinc-800 text-white w-60"
                min={1}
              />
            </div>
            ...
            <div className="space-y-2">
              <label className="block font-medium">
                Deseja instalar proteções?
              </label>
              <label className="flex items-center">
                <input
                  type="checkbox"
                  name="protection"
                  value="Disjuntor exclusivo"
                  className="mr-2"
                  required
                />
                Disjuntor exclusivo
            ...
            <textarea
              ...
              className="p-2 rounded border border-zinc-600 bg-zinc-800 text-white w-full"
              rows={3}
            />
